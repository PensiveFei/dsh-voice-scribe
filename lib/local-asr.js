...
async function downloadFile(mirrors, filename, targetDir, onProgress) {
	...
	for (const mirror of mirrors) {
		...
		try {
			...
			try {
				await stat(part).then(() => rename(part, join(targetDir, filename + ".part.fail")));
			} catch {
				/* no partial */
			}
		}
 ...
 */
export async function startModelDownload(dir = modelDir()) {
	...
	void (async () => {
		try {
			...
			for (const file of ["tokens.txt", "model.int8.onnx"]) {
				...
				const result = await downloadFile(MODEL_MIRRORS, file, dir, (f, done, total) => {
...
export async function ensureRecognizer(dir = modelDir()) {
	...
	recognizer = await loadRecognizer(dir);
	recognizerDir = dir;
	return recognizer;
}
