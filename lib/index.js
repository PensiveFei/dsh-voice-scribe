// SPDX-License-Identifier: MIT
// ============================================================================
// dsh-voice-scribe — host half.
//
// Routes (fenced to loopback / trusted hosts, same-origin only):
//   POST /voice-input/transcribe   { audio: base64, mimeType, language? } -> { ok, text }
//        decodes audio, POSTs it to the configured ASR endpoint
//        (OpenAI-compatible /v1/audio/transcriptions), returns the text.
//   POST /voice-input/polish       { text, provider, model } -> { ok, text }
//        optionally re-runs the transcript through the DSH-configured LLM
//        (ctx.llm) to strip fillers / fix punctuation. Zero extra config:
//        provider + model are picked from DSH's own model list.
//
// The browser half keeps its settings (ASR base URL / key / language / hotkey
// / polish on-off) in localStorage; the API key is NEVER sent to the browser.
// The browser posts only the audio + the chosen language; the host resolves
// the key from its own settings scope (host-side, not exposed to the page).
//
// Dependency-free helpers (settings, trust fence, body reading, ASR call)
// live in ./host-utils.js so the offline test suite can import them directly.
// @deepseek-ai/dsh-llm is lazy-imported inside polishText (never at module
// load), so lib/index.js itself is importable in plain Node for tests too.
// ============================================================================

import {
	DEFAULT_ASR_URL,
	DEFAULT_ASR_MODEL,
	DEFAULT_POLISH_PROMPT,
	MAX_ASR_PROVIDERS,
	MAX_POLISH_PROMPT_CHARS,
	PAYLOAD_TOO_LARGE,
	applyHotwords,
	buildTrustedHosts,
	hotwordsPath,
	isTrustedApiRequest,
	loadHotwords,
	localPolish,
	readJsonBody,
	readSettings,
	resolveAsrProviders,
	resolvePolishPrompt,
	safeMessage,
	transcribeAudio,
	writeJson,
	writeSettings
} from "./host-utils.js";
import {
	base64ToFloat32,
	getDownloadState,
	modelDir,
	modelReady,
	startModelDownload,
	transcribePcm
} from "./local-asr.js";

/** Plugin identity for cordis.yml rows. */
export const name = "dsh-voice-scribe";
/** Services required before mounting: web routes, trust fence host list, and the DSH LLM runtime. */
export const inject = ["webServer", "webRuntime", "llm"];

/** Route prefix owned by this plugin. */
const API_PREFIX = "/voice-input";
/** Max transcript chars accepted into the LLM polish call. */
const MAX_TRANSCRIPT_CHARS = 12_000;
/** Timeout for one polish call. */
const POLISH_TIMEOUT_MS = 30_000;

// ── LLM polish (reuses DSH's configured models — zero extra config) ────────

/**
 * Polish a transcript through the DSH-configured LLM route (ctx.llm).
 * Provider + model are chosen by the user from DSH's own model list; the API
 * key comes from DSH's provider credentials — nothing new to configure.
 * Any failure (timeout / route error / stream error) keeps the raw transcript.
 */
export async function polishText({ text, provider, model, ctx, signal, prompt }) {
	const raw = text.trim();
	if (raw === "" || raw.length > MAX_TRANSCRIPT_CHARS) return raw;
	// Local pre-polish: strip fillers first so the LLM gets shorter, cleaner input.
	const cleaned = localPolish(raw);
	if (cleaned === "") return raw;

	const timeout = new AbortController();
	const timer = setTimeout(() => timeout.abort(), POLISH_TIMEOUT_MS);
	const forwardAbort = () => timeout.abort(signal && signal.reason);
	if (signal) signal.addEventListener("abort", forwardAbort, { once: true });
	// An AbortSignal that is ALREADY aborted never fires "abort" again for a
	// listener attached afterwards — forward the existing state explicitly,
	// otherwise a client that hung up before the call still burns the full
	// 30 s polish timeout (the same trap as the ASR path in host-utils).
	if (signal && signal.aborted) timeout.abort(signal.reason);

	try {
		const prepared = await ctx.llm.prepareCall({ provider, model }, timeout.signal);
		// Lazy-import: DSH provides @deepseek-ai/dsh-llm at runtime, but the
		// module must stay importable in plain Node so the offline test suite
		// can exercise polishText directly. The import only runs on the first
		// real polish call and is cached by Node afterwards.
		const { createUserMessage } = await import("@deepseek-ai/dsh-llm");
		const message = createUserMessage({
			content: [{ type: "text", text: cleaned }],
			source: { kind: "user" }
		});
		// Custom polish prompt (Settings → 语音输入) wins over the built-in
		// default; resolvePolishPrompt owns the same fallback for the route.
		const system = typeof prompt === "string" && prompt.trim() !== "" ? prompt.trim() : DEFAULT_POLISH_PROMPT;

		let out = "";
		for await (const chunk of prepared.stream({
			...prepared.config,
			messages: [message],
			system,
			signal: timeout.signal
		})) {
			if (chunk.type === "text-delta" && typeof chunk.text === "string") out += chunk.text;
			else if (chunk.type === "finish" && (chunk.reason.kind === "error" || chunk.reason.kind === "aborted")) {
				const error = new Error("polish route failed");
				error.code = "polish-route";
				throw error;
			}
		}
		const result = out.trim();
		return result === "" ? raw : result;
	} catch (error) {
		if (error && error.name === "AbortError") return raw; // timeout → keep raw transcript
		if (error && error.code === "polish-route") return raw; // route error → keep raw
		return raw; // any polish failure → keep the raw transcript (never lose the user's words)
	} finally {
		clearTimeout(timer);
		if (signal) signal.removeEventListener("abort", forwardAbort);
	}
}

// ── request handling ───────────────────────────────────────────────────────

export async function handleApi(req, res, ctx) {
	if (req.method !== "POST") {
		writeJson(res, 405, { ok: false, error: { code: "method-error", message: "method not allowed" } });
		return;
	}
	const contentType = typeof req.headers["content-type"] === "string" ? req.headers["content-type"].toLowerCase() : "";
	if (!contentType.startsWith("application/json")) {
		writeJson(res, 415, { ok: false, error: { code: "unsupported-media-type", message: "content-type must be application/json" } });
		return;
	}
	const payload = await readJsonBody(req);
	if (payload === PAYLOAD_TOO_LARGE) {
		// The request body was not fully consumed — close the connection after
		// the 413 so the socket does not linger half-drained.
		res.setHeader("connection", "close");
		writeJson(res, 413, { ok: false, error: { code: "payload-too-large", message: "request body too large" } });
		return;
	}
	if (payload === null || typeof payload !== "object" || typeof payload.action !== "string") {
		writeJson(res, 400, { ok: false, error: { code: "bad-request", message: "bad request" } });
		return;
	}

	// Abort wiring: if the browser disconnects mid-request, cancel the
	// in-flight ASR / polish call instead of letting it run to its timeout.
	const abort = new AbortController();
	const onDisconnect = () => abort.abort();
	req.on("aborted", onDisconnect);
	res.on("close", () => {
		if (!res.writableEnded) onDisconnect();
	});

	// GET-style settings read: exposes only non-secret view (never the key).
	if (payload.action === "get-settings") {
		const settings = readSettings();
		const providers = resolveAsrProviders(settings).map((p) => ({
			url: p.url,
			model: p.model,
			hasKey: p.key !== ""
		}));
		const hotwords = loadHotwords();
		writeJson(res, 200, {
			ok: true,
			value: {
				asrUrl: (settings.asrUrl || DEFAULT_ASR_URL),
				asrModel: (settings.asrModel || DEFAULT_ASR_MODEL),
				language: settings.language || "",
				// Type-guarded like every other field here: a hand-edited voice-input.json
				// with a non-string key used to throw straight out of the handler, and the
				// 500 left the whole 语音输入 settings row blank with no error shown.
				hasKey: typeof settings.asrApiKey === "string" && settings.asrApiKey.trim() !== "",
				providers,
				// Custom polish prompt ("" = built-in default in use).
				polishPrompt: typeof settings.polishPrompt === "string" ? settings.polishPrompt : "",
				// Hot-word table status (path + rule count; never its content).
				hotwords: {
					path: hotwordsPath(),
					rules: hotwords.rules.length,
					error: hotwords.errors.length > 0 ? hotwords.errors[0].line + ": " + hotwords.errors[0].message : ""
				}
			}
		});
		return;
	}

	// Settings write: accepts only the safe fields; the key is stored host-side.
	// Empty/whitespace string values DELETE the field (lets the UI clear the
	// stored key); asrUrl must be an http(s) URL.
	if (payload.action === "set-settings") {
		const patch = payload.patch;
		if (patch === null || typeof patch !== "object" || Array.isArray(patch)) {
			writeJson(res, 400, { ok: false, error: { code: "bad-request", message: "patch must be a plain object" } });
			return;
		}
		const next = readSettings();
		for (const [key, value] of Object.entries(patch)) {
			if (key !== "asrUrl" && key !== "asrModel" && key !== "language" && key !== "asrApiKey" && key !== "asrProviders" && key !== "polishPrompt") continue;
			// Polish prompt: multi-line is fine, but it must stay small enough
			// that the LLM call still has room for the transcript itself.
			if (key === "polishPrompt" && typeof value === "string" && value.trim().length > MAX_POLISH_PROMPT_CHARS) {
				writeJson(res, 400, { ok: false, error: { code: "bad-request", message: "polishPrompt is too long (max " + MAX_POLISH_PROMPT_CHARS + " chars)" } });
				return;
			}
			if (key === "asrProviders") {
				if (value === null) { delete next.asrProviders; continue; }
				if (!Array.isArray(value)) {
					writeJson(res, 400, { ok: false, error: { code: "bad-request", message: "asrProviders must be an array" } });
					return;
				}
				// Key preservation map: rows saved WITHOUT an explicit key
				// inherit the stored key of the SAME url from the previous
				// settings (the array is replaced wholesale, so absent key
				// must not silently drop the old one).
				const prevProviders = new Map();
				if (Array.isArray(next.asrProviders)) {
					for (const old of next.asrProviders) {
						if (old && typeof old === "object" && typeof old.url === "string" && old.url.trim() !== "") {
							prevProviders.set(old.url.trim(), old);
						}
					}
				}
				if (typeof next.asrUrl === "string" && next.asrUrl.trim() !== "") {
					prevProviders.set(next.asrUrl.trim(), { url: next.asrUrl.trim(), key: next.asrApiKey });
				}
				const providers = [];
				for (const p of value) {
					if (p === null || typeof p !== "object" || Array.isArray(p)) continue;
					const url = typeof p.url === "string" ? p.url.trim() : "";
					if (url === "") continue;
					if (!/^https?:\/\//i.test(url)) {
						writeJson(res, 400, { ok: false, error: { code: "bad-request", message: "asrProviders URLs must be http(s)" } });
						return;
					}
					const model = typeof p.model === "string" ? p.model.trim() : "";
					const entry = { url, model };
					// Key is write-only:
					//  - absent / null  → keep whatever the host already stored
					//  - "" (empty)     → DELETE the stored key for this row
					//  - non-empty      → set the key
					if (typeof p.key === "string") {
						if (p.key.trim() !== "") entry.key = p.key.trim();
						else entry.key = null; // explicit delete marker
					} else {
						// Absent key: inherit the stored key for this url
						// (the array is replaced wholesale, so absent key
						// must not silently drop the old one).
						const prev = prevProviders.get(url);
						if (prev && typeof prev.key === "string" && prev.key !== "") entry.key = prev.key;
					}
					providers.push(entry);
					if (providers.length >= MAX_ASR_PROVIDERS) break;
				}
				next.asrProviders = providers;
				continue;
			}
			if (value === null) {
				delete next[key];
				continue;
			}
			if (typeof value !== "string") continue;
			const trimmed = value.trim();
			if (key === "asrUrl" && trimmed !== "" && !/^https?:\/\//i.test(trimmed)) {
				writeJson(res, 400, { ok: false, error: { code: "bad-request", message: "asrUrl must be an http(s) URL" } });
				return;
			}
			if (trimmed === "") delete next[key];
			else next[key] = trimmed;
		}
		writeSettings(next);
		writeJson(res, 200, { ok: true });
		return;
	}

	// Transcribe: { audio: base64, mimeType, language? }
	if (payload.action === "transcribe") {
		const audio = typeof payload.audio === "string" ? payload.audio : "";
		const mimeType = typeof payload.mimeType === "string" ? payload.mimeType : "audio/webm";
		const language = typeof payload.language === "string" ? payload.language : "";
		const settings = readSettings();
		try {
			const text = await transcribeAudio({ audioBase64: audio, mimeType, language, settings, signal: abort.signal });
			// Hot-word replacement table (hot.txt) applies to every engine.
			writeJson(res, 200, { ok: true, text: applyHotwords(text, loadHotwords().rules) });
		} catch (error) {
			console.error("[dsh-voice-scribe] transcribe failed:", safeMessage(error && error.message));
			writeJson(res, 200, { ok: false, error: { code: error && error.code || "transcribe-failed", message: safeMessage(error && error.message) } });
		}
		return;
	}

	// Polish: { text, provider, model }
	if (payload.action === "polish") {
		const text = typeof payload.text === "string" ? payload.text : "";
		const provider = typeof payload.provider === "string" ? payload.provider : "";
		const model = typeof payload.model === "string" ? payload.model : "";
		if (text === "" || provider === "" || model === "") {
			writeJson(res, 400, { ok: false, error: { code: "bad-request", message: "text, provider and model are required" } });
			return;
		}
		try {
			const polished = await polishText({ text, provider, model, ctx, signal: abort.signal, prompt: resolvePolishPrompt(readSettings()) });
			writeJson(res, 200, { ok: true, text: polished });
		} catch (error) {
			console.error("[dsh-voice-scribe] polish failed:", safeMessage(error && error.message));
			// Never fail the flow on polish: the browser already has the raw transcript.
			writeJson(res, 200, { ok: false, error: { code: error && error.code || "polish-failed", message: safeMessage(error && error.message) } });
		}
		return;
	}

	// Local offline ASR (SenseVoice via sherpa-onnx) — zero config, zero key.
	if (payload.action === "local-status") {
		const dir = modelDir();
		const state = getDownloadState();
		writeJson(res, 200, {
			ok: true,
			value: {
				modelReady: await modelReady(dir),
				modelDir: dir,
				downloading: state.running,
				progress: state.progress,
				error: state.error
			}
		});
		return;
	}

	// Start (or resume) the background model download. Idempotent.
	if (payload.action === "local-download") {
		try {
			const result = await startModelDownload();
			writeJson(res, 200, { ok: true, ...result });
		} catch (error) {
			console.error("[dsh-voice-scribe] local model download failed:", safeMessage(error && error.message));
			writeJson(res, 200, { ok: false, error: { code: error && error.code || "download-failed", message: safeMessage(error && error.message) } });
		}
		return;
	}

	// Transcribe 16 kHz mono float32 PCM (base64 little-endian bytes).
	if (payload.action === "local-transcribe") {
		const audio = typeof payload.audio === "string" ? payload.audio : "";
		const sampleRate = typeof payload.sampleRate === "number" && payload.sampleRate > 0 ? payload.sampleRate : 16000;
		if (audio === "") {
			writeJson(res, 400, { ok: false, error: { code: "bad-request", message: "audio is required" } });
			return;
		}
		try {
			const text = await transcribePcm(base64ToFloat32(audio), sampleRate, modelDir(), abort.signal);
			writeJson(res, 200, { ok: true, text: applyHotwords(text, loadHotwords().rules) });
		} catch (error) {
			console.error("[dsh-voice-scribe] local transcribe failed:", safeMessage(error && error.message));
			writeJson(res, 200, { ok: false, error: { code: error && error.code || "local-asr-failed", message: safeMessage(error && error.message) } });
		}
		return;
	}

	// List DSH's configured LLM routes for the polish picker (no secrets).
	if (payload.action === "list-models") {
		const routes = [];
		for (const provider of ctx.llm.listProviders()) {
			let models = [];
			try {
				models = await ctx.llm.listModels(provider.id);
			} catch {
				continue;
			}
			for (const model of models) {
				routes.push({ provider: provider.id, providerName: provider.name, model: model.id, modelName: model.name });
			}
		}
		writeJson(res, 200, { ok: true, value: routes });
		return;
	}

	writeJson(res, 404, { ok: false, error: { code: "not-found", message: "unknown action " + JSON.stringify(payload.action) } });
}

/** Host loader entry: mount the fenced voice-input API. */
export function apply(ctx) {
	// Validate/filter trustedHosts ONCE at startup — a bad entry must not
	// throw inside the per-request handler (which would 500 every call).
	const trustedHosts = buildTrustedHosts(ctx.webRuntime.trustedHosts, (message) => {
		console.warn("[dsh-voice-scribe] ignoring invalid trustedHosts entry:", message);
	});

	ctx.effect(() => ctx.webServer.register({
		kind: "prefix",
		path: API_PREFIX,
		handler: async (req, res) => {
			if (!isTrustedApiRequest(req, trustedHosts)) {
				writeJson(res, 403, { ok: false, error: { code: "forbidden", message: "forbidden" } });
				return;
			}
			try {
				await handleApi(req, res, ctx);
			} catch (error) {
				// Never echo internal errors back to the browser.
				console.error("[dsh-voice-scribe] API error:", error);
				writeJson(res, 500, { ok: false, error: { code: "internal", message: "internal error" } });
			}
		}
	}), "dsh-voice-scribe: API routes");
}